...
class MonitorKoordinatorTambah extends Component {
  constructor(props) {
    super(props);
    this.state = {
      IsiMonitor: '',
  ...
  createMonitor = isiMonitor => {
    ...
    this.props.navigation.dispatch(StackActions.replace('MonitorKoordinator'));
  ...
  getData = async () => {
    try {
      let value = await AsyncStorage.getItem('@bmonitor');
...
export default MonitorKoordinatorTambah;
